...
% Part of QWTB. Wrapper script for algorithm TWM-MPItest.
% Format input data --------------------------- %<<<1

% just for test - store datain as .original
datainoriginal = datain;
for k = 1:3
    % find quantities for k and save them to datain
    datain = struct();
    numstr = sprintf('%d',k);
    Qnames = fieldnames(datainoriginal);
    numlen = numel(numstr);
    ...
    % next part is calculated as always
...
%     u_PF = ((u_P./P).^2 + (u_S./S).^2).^0.5; % ###note: ignoring corelations, may be improved
    ...
    % returned is only last one, it is only for test!
end % for k - for every phase
